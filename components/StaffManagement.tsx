...
import { getStoredUsers, createUser } from '../store';
...
export const StaffManagement: React.FC = () => {
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    setNewUser({ name: '', email: '', password: 'password123', role: Role.SALES, sector: ServiceSector.NONE });
  };

  const removeUser = async (id: string) => {
    if (confirm('Are you sure you want to remove this team member?')) {
      // For now, just reload - delete endpoint would be added to backend
      await loadUsers();
    }
  };
